...
public class MasterValidator {
    ...
    public void checkIfAlreadyExists(String username, String email) {
        String message = null;
        if(masterRepository.findIfExistsByUsername(username)) message = "Username Already Exists!";
        if(masterRepository.findIfExistsByEmail(email)) {
            if(message != null) {
    ...
    public Master checkIfAlreadyExists(String input) {
        if (checkEmail(input))
            return masterRepository.findByEmail(input);
        else
            return masterRepository.findByUsername(input);
    }
